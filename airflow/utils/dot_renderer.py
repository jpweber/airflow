...
def render_dag(dag: DAG, tis: Optional[List[TaskInstance]] = None) -> graphviz.Digraph:
    ...
    for task in dag.tasks:
        ...
        if states_by_task_id is None:
            node_attrs.update(
                {"color": _refine_color(task.ui_fgcolor), "fillcolor": _refine_color(task.ui_color),}
            )
        else:
            state = states_by_task_id.get(task.task_id, State.NONE)
            node_attrs.update(
                {"color": State.color_fg(state), "fillcolor": State.color(state),}
            )
        dot.node(
            task.task_id, _attributes=node_attrs,
        )
