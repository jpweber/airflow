...
class WorkerConfiguration(LoggingMixin):
    ...
    def _get_init_containers(self) -> List[k8s.V1Container]:
        ...
        if (
            ...
            or self.kube_config.dags_volume_host
            or self.kube_config.dags_in_image
        ):
            return []

        # Otherwise, define a git-sync init container
        init_environment = [
            k8s.V1EnvVar(name='GIT_SYNC_REPO', value=self.kube_config.git_repo),
            k8s.V1EnvVar(name='GIT_SYNC_BRANCH', value=self.kube_config.git_branch),
            k8s.V1EnvVar(name='GIT_SYNC_ROOT', value=self.kube_config.git_sync_root),
            k8s.V1EnvVar(name='GIT_SYNC_DEST', value=self.kube_config.git_sync_dest),
            k8s.V1EnvVar(name='GIT_SYNC_REV', value=self.kube_config.git_sync_rev),
            k8s.V1EnvVar(name='GIT_SYNC_DEPTH', value=self.kube_config.git_sync_depth),
            k8s.V1EnvVar(name='GIT_SYNC_ONE_TIME', value='true'),
        ]
        if self.kube_config.git_user:
            init_environment.append(k8s.V1EnvVar(name='GIT_SYNC_USERNAME', value=self.kube_config.git_user))
        if self.kube_config.git_password:
            init_environment.append(
                k8s.V1EnvVar(name='GIT_SYNC_PASSWORD', value=self.kube_config.git_password)
            )

        volume_mounts = [
            k8s.V1VolumeMount(
                mount_path=self.kube_config.git_sync_root, name=self.dags_volume_name, read_only=False
            )
        ]
                    ...
                    k8s.V1EnvVar(
                        ...
                        value_from=k8s.V1EnvVarSource(
                            secret_key_ref=k8s.V1SecretKeySelector(
                                name=self.kube_config.git_sync_credentials_secret, key='GIT_SYNC_USERNAME'
                            )
                        ),
                    ),
                    k8s.V1EnvVar(
                        ...
                        value_from=k8s.V1EnvVarSource(
                            secret_key_ref=k8s.V1SecretKeySelector(
                                name=self.kube_config.git_sync_credentials_secret, key='GIT_SYNC_PASSWORD'
                            )
                        ),
                    ),
                ]
        ...
        if self.kube_config.git_ssh_key_secret_name:
            ...
            init_environment.extend(
                [
                    ...
                    k8s.V1EnvVar(name='GIT_SYNC_ADD_USER', value='true'),
        ...
        if self.kube_config.git_ssh_known_hosts_configmap_name:
            ...
            init_environment.extend(
                [
                    ...
                    k8s.V1EnvVar(name='GIT_SSH_KNOWN_HOSTS_FILE', value='/etc/git-secret/known_hosts'),
                ]
            )
        else:
            init_environment.append(k8s.V1EnvVar(name='GIT_KNOWN_HOSTS', value='false'))
    ...
    def _get_environment(self) -> Dict[str, str]:
        ...
        if (
            not self.kube_config.airflow_configmap
            and 'AIRFLOW__CORE__SQL_ALCHEMY_CONN' not in self.kube_config.kube_secrets
        ):
    ...
    def _get_env_from(self) -> List[k8s.V1EnvFromSource]:
        ...
        if self.kube_config.env_from_configmap_ref:
            for config_map_ref in self.kube_config.env_from_configmap_ref.split(','):
                env_from.append(k8s.V1EnvFromSource(config_map_ref=k8s.V1ConfigMapEnvSource(config_map_ref)))

        if self.kube_config.env_from_secret_ref:
            for secret_ref in self.kube_config.env_from_secret_ref.split(','):
                env_from.append(k8s.V1EnvFromSource(secret_ref=k8s.V1SecretEnvSource(secret_ref)))
    ...
    def _get_volume_mounts(self) -> List[k8s.V1VolumeMount]:
        volume_mounts = {
            self.dags_volume_name: k8s.V1VolumeMount(
                name=self.dags_volume_name, mount_path=self.generate_dag_volume_mount_path(), read_only=True,
            ),
            self.logs_volume_name: k8s.V1VolumeMount(
                name=self.logs_volume_name, mount_path=self.worker_airflow_logs,
            ),
        }
        ...
        if self.kube_config.airflow_configmap:
            ...
            volume_mounts[config_volume_name] = k8s.V1VolumeMount(
                name=config_volume_name, mount_path=config_path, sub_path='airflow.cfg', read_only=True
            )
    ...
    def _get_volumes(self) -> List[k8s.V1Volume]:
        def _construct_volume(name, claim, host) -> k8s.V1Volume:
            ...
            if claim:
                volume.persistent_volume_claim = k8s.V1PersistentVolumeClaimVolumeSource(claim_name=claim)
            elif host:
                volume.host_path = k8s.V1HostPathVolumeSource(path=host, type='')
            else:
                volume.empty_dir = {}
        ...
        volumes = {
            self.dags_volume_name: _construct_volume(
                self.dags_volume_name, self.kube_config.dags_volume_claim, self.kube_config.dags_volume_host
            ),
            self.logs_volume_name: _construct_volume(
                self.logs_volume_name, self.kube_config.logs_volume_claim, self.kube_config.logs_volume_host
            ),
        }
        ...
        if self.kube_config.git_ssh_key_secret_name:
            volumes[self.git_sync_ssh_secret_volume_name] = k8s.V1Volume(
                ...
                secret=k8s.V1SecretVolumeSource(
                    ...
                    items=[k8s.V1KeyToPath(key=self.git_ssh_key_secret_key, path='ssh', mode=0o440)],
                ),
            )
        ...
        if self.kube_config.git_ssh_known_hosts_configmap_name:
            volumes[self.git_sync_ssh_known_hosts_volume_name] = k8s.V1Volume(
                ...
                config_map=k8s.V1ConfigMapVolumeSource(
                    name=self.kube_config.git_ssh_known_hosts_configmap_name, default_mode=0o440
                ),
            )
        ...
        if self.kube_config.airflow_configmap:
            ...
            volumes[config_volume_name] = k8s.V1Volume(
                name=config_volume_name,
                config_map=k8s.V1ConfigMapVolumeSource(name=self.kube_config.airflow_configmap),
            )
