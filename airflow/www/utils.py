...
def generate_pages(current_page, num_of_pages, search=None, status=None, window=7):
    ...
    output.append(
        first_node.format(
            href_link="?{}".format(get_params(page=0, search=search, status=status)), disabled=is_disabled
        )
    )
    ...
    output.append(
        last_node.format(
            href_link="?{}".format(get_params(page=last_page, search=search, status=status)),
...
def state_token(state):
    ...
    return Markup('<span class="label" style="background-color:{color};">' '{state}</span>').format(
...
def wrapped_markdown(s, css_class=None):
    ...
    return Markup('<div class="rich_doc {css_class}" >' + markdown.markdown(s) + "</div>").format(
        css_class=css_class
    )
