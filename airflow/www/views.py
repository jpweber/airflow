...
def get_date_time_num_runs_dag_runs_form_data(request, session, dag):
    ...
    drs = (
        session.query(DR)
        .filter(DR.dag_id == dag.dag_id, DR.execution_date <= base_date)
...
class Airflow(AirflowBaseView):  # noqa: D101
    ...
    def index(self):
        ...
        for filename, stacktrace in plugin_import_errors.items():
            flash(
                "Broken plugin: [{filename}] {stacktrace}".format(stacktrace=stacktrace, filename=filename),
                "error",
            )
    ...
    def task_stats(self, session=None):
        ...
        RunningTI = session.query(TI.dag_id.label('dag_id'), TI.state.label('state')).join(
            RunningDagRun,
            and_(RunningDagRun.c.dag_id == TI.dag_id, RunningDagRun.c.execution_date == TI.execution_date),
        )
        if selected_dag_ids:
        ...
        if conf.getboolean('webserver', 'SHOW_RECENT_STATS_FOR_COMPLETED_RUNS', fallback=True):
            ...
            LastTI = session.query(TI.dag_id.label('dag_id'), TI.state.label('state')).join(
                LastDagRun,
                and_(LastDagRun.c.dag_id == TI.dag_id, LastDagRun.c.execution_date == TI.execution_date),
            )
            if selected_dag_ids:
        ...
        qry = session.query(FinalTI.c.dag_id, FinalTI.c.state, sqla.func.count()).group_by(
            FinalTI.c.dag_id, FinalTI.c.state
        )
    ...
    def last_dagruns(self, session=None):
        ...
        query = session.query(DagRun.dag_id, sqla.func.max(DagRun.execution_date).label('last_run')).group_by(
            DagRun.dag_id
        )
        ...
        resp = {
            r.dag_id.replace('.', '__dot__'): {'dag_id': r.dag_id, 'last_run': r.last_run.isoformat(),}
            for r in query
        }
    ...
    def trigger(self, session=None):
        ...
        if request.method == 'GET':
            return self.render_template('airflow/trigger.html', dag_id=dag_id, origin=origin, conf='')
        ...
        if conf:
            try:
                ...
                return self.render_template('airflow/trigger.html', dag_id=dag_id, origin=origin, conf=conf)
    ...
    def blocked(self, session=None):
        ...
        for dag_id, active_dag_runs in dags:
            ...
            payload.append(
                {'dag_id': dag_id, 'active_dag_run': active_dag_runs, 'max_active_runs': max_active_runs,}
            )
        return wwwutils.json_response(payload)
    ...
    def _mark_task_instance_state(
        self, dag_id, task_id, origin, execution_date, confirmed, upstream, downstream, future, past, state
    ):
        dag = current_app.dag_bag.get_dag(dag_id)
        task = dag.get_task(task_id)
        task.dag = dag
    ...
    def tree(self):
        ...
        root = request.args.get('root')
        if root:
            dag = dag.sub_dag(task_regex=root, include_downstream=False, include_upstream=True)
    ...
    def graph(self, session=None):
        ...
        tasks = {
            t.task_id: {'dag_id': t.dag_id, 'task_type': t.task_type, 'extra_links': t.extra_links,}
            for t in dag.tasks
        }
        if not tasks:
    ...
    def duration(self, session=None):
        ...
        cum_chart.create_y_axis(
            'yAxis', format='.02f', custom_format=False, label='Duration ({})'.format(cum_y_unit)
        )
        ...
        for task in dag.tasks:
            if x[task.task_id]:
                chart.add_serie(
                    name=task.task_id, x=x[task.task_id], y=scale_time_units(y[task.task_id], y_unit)
                )
                cum_chart.add_serie(
                    name=task.task_id, x=x[task.task_id], y=scale_time_units(cum_y[task.task_id], cum_y_unit)
                )
    ...
    def landing_times(self, session=None):
        ...
        chart.create_y_axis(
            'yAxis', format='.02f', custom_format=False, label='Landing Time ({})'.format(y_unit)
        )
        chart.axislist['yAxis']['axisLabelDistance'] = '-15'
        for task in dag.tasks:
            if x[task.task_id]:
    ...
    def extra_links(self):
        ...
        if not dag or task_id not in dag.task_ids:
            response = jsonify(
                {
                    'url': None,
                    'error': "can't find dag {dag} or task_id {task_id}".format(dag=dag, task_id=task_id),
                }
            )
...
class VersionView(AirflowBaseView):
    ...
    def version(self):
        ...
        title = "Version Info"
        return self.render_template(
            'airflow/version.html', title=title, airflow_version=airflow_version, git_version=git_version
        )
...
class ConfigurationView(AirflowBaseView):
    ...
    def conf(self):
        ...
        if raw:
            ...
            code_html = Markup(
                highlight(config, lexers.IniLexer(), HtmlFormatter(noclasses=True))  # Lexer call
            )
            return self.render_template(
                'airflow/config.html',
...
class ConnectionModelView(AirflowModelView):
    ...
    def prefill_form(self, form, pk):
        ...
        if not hasattr(d, 'get'):
            logging.warning(
                'extra field for {} is not iterable'.format(form.data.get('conn_id', '<unknown>'))
            )
            return
