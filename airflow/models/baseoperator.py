...
class BaseOperator(Operator, LoggingMixin, metaclass=BaseOperatorMeta):
        ...
        **kwargs,
        ...
        if args or kwargs:
            ...
            warnings.warn(
                ...
                '\n*args: {a}\n**kwargs: {k}'.format(c=self.__class__.__name__, a=args, k=kwargs, t=task_id),
        ...
        if self.pool_slots < 1:
            raise AirflowException(
                "pool slots for %s in dag %s cannot be less than 1" % (self.task_id, dag.dag_id)
            )
        self.sla = sla
        ...
        if inlets:
            self._inlets = (
                inlets if isinstance(inlets, list) else [inlets,]
            )

        if outlets:
            self._outlets = (
                outlets if isinstance(outlets, list) else [outlets,]
            )
    ...
    def get_flat_relative_ids(
        self, upstream: bool = False, found_descendants: Optional[Set[str]] = None
    ) -> Set[str]:
        """
    ...
    def _set_relatives(
        self, task_or_task_list: Union['BaseOperator', List['BaseOperator']], upstream: bool = False
    ) -> None:
    ...
    @staticmethod
    def xcom_push(context: Any, key: str, value: Any, execution_date: Optional[datetime] = None,) -> None:
        """
...
def cross_downstream(from_tasks: List[BaseOperator], to_tasks: Union[BaseOperator, List[BaseOperator]]):
    r"""
