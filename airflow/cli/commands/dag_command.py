...
def _tabulate_dag_runs(dag_runs: List[DagRun], tablefmt: str = "fancy_grid") -> str:
    ...
    return tabulate(tabular_data=tabulat_data, tablefmt=tablefmt)


def _tabulate_dags(dags: List[DAG], tablefmt: str = "fancy_grid") -> str:
    tabulat_data = (
        {'DAG ID': dag.dag_id, 'Filepath': dag.filepath, 'Owner': dag.owner,}
        for dag in sorted(dags, key=lambda d: d.dag_id)
    )
    return tabulate(tabular_data=tabulat_data, tablefmt=tablefmt, headers='keys')
