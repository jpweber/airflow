...
class TestCliDags(unittest.TestCase):
    ...
    def test_next_execution(self):
        ...
        # Test None output
        args = self.parser.parse_args(['dags', 'next_execution', dag_ids[0]])
        with contextlib.redirect_stdout(io.StringIO()) as temp_stdout:
        ...
        for i, dag_id in enumerate(dag_ids):
            ...
            args = self.parser.parse_args(['dags', 'next_execution', dag_id])
            with contextlib.redirect_stdout(io.StringIO()) as temp_stdout:
                dag_command.dag_next_execution(args)
                out = temp_stdout.getvalue()
            ...
            args = self.parser.parse_args(['dags', 'next_execution', dag_id, '--num-executions', '2'])
            with contextlib.redirect_stdout(io.StringIO()) as temp_stdout:
                dag_command.dag_next_execution(args)
                out = temp_stdout.getvalue()
