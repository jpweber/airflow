...
class TestTaskInstance(unittest.TestCase):
    ...
    def test_not_requeue_non_requeueable_task_instance(self):
        ...
        for dep in all_non_requeueable_deps:
            ...
            dep_patch = patch('%s.%s.%s' % (dep.__module__, class_name, dep._get_dep_statuses.__name__))
    ...
    @parameterized.expand(
        [(False, None,), (True, {'env': None, 'bash_command': 'echo test_retry_handling; exit 1'},),]
    )
    ...
    def test_depends_on_past(self):
        ...
        task = DummyOperator(task_id='test_dop_task', dag=dag, depends_on_past=True,)
        dag.clear()
    ...
    def test_previous_start_date_success(self, _, schedule_interval, catchup) -> None:
        ...
        self.assertEqual(
            ti_list[3].get_previous_start_date(state=State.SUCCESS), ti_list[1].start_date,
        )
        self.assertNotEqual(
            ti_list[3].get_previous_start_date(state=State.SUCCESS), ti_list[2].start_date,
        )
...
class TestRunRawTaskQueriesCount(unittest.TestCase):
    ...
    def test_execute_queries_count_store_serialized(self):
        ...
        with assert_queries_count(10), patch("airflow.models.taskinstance.STORE_SERIALIZED_DAGS", True):
            ti._run_raw_task()
