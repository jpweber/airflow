...
class TestFileSensor(unittest.TestCase):
    ...
    def test_simple(self):
        with tempfile.NamedTemporaryFile() as tmp:
            task = FileSensor(
                task_id="test", filepath=tmp.name[1:], fs_conn_id='fs_default', dag=self.dag, timeout=0,
            )
    ...
    def test_file_in_dir(self):
        temp_dir = tempfile.mkdtemp()
        task = FileSensor(
            task_id="test", filepath=temp_dir[1:], fs_conn_id='fs_default', dag=self.dag, timeout=0,
        )
        task._hook = self.hook
        try:
            # `touch` the dir
    ...
    def test_default_fs_conn_id(self):
        with tempfile.NamedTemporaryFile() as tmp:
            task = FileSensor(task_id="test", filepath=tmp.name[1:], dag=self.dag, timeout=0,)
            task._hook = self.hook
            task.run(start_date=DEFAULT_DATE, end_date=DEFAULT_DATE, ignore_ti_state=True)
    ...
    def test_wildcard_file(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=suffix) as tmp:
            ...
            task = FileSensor(
                task_id='test', filepath=fileglob, fs_conn_id='fs_default', dag=self.dag, timeout=0,
            )
            task._hook = self.hook
            task.run(start_date=DEFAULT_DATE, end_date=DEFAULT_DATE, ignore_ti_state=True)
    ...
    def test_subdirectory_not_empty(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=suffix, dir=subdir):
            task = FileSensor(
                task_id='test', filepath=temp_dir, fs_conn_id='fs_default', dag=self.dag, timeout=0,
            )
            task._hook = self.hook
            task.run(start_date=DEFAULT_DATE, end_date=DEFAULT_DATE, ignore_ti_state=True)
        shutil.rmtree(temp_dir)
