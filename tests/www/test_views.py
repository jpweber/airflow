...
class TestVariableModelView(TestBase):
    ...
    def test_can_handle_error_on_decrypt(self):
        ...
        resp = self.client.post('/variable/add', data=self.variable, follow_redirects=True)
    ...
    def test_xss_prevention(self):
        ...
        resp = self.client.get(xss, follow_redirects=True,)
...
class TestAirflowBaseViews(TestBase):
    ...
    def test_run_with_runnable_states(self, get_default_executor_function):
        ...
        for state in RUNNABLE_STATES:
            ...
            msg = (
                "Task is in the &#39;{}&#39; state which is not a valid state for execution. ".format(state)
    ...
    def test_version(self):
        with self.capture_templates() as templates:
            resp = self.client.get(
                'version', data=dict(username='test', password='test'), follow_redirects=True
            )
        ...
        self.assertEqual(
            ...
            dict(airflow_version=version.version, git_version=mock.ANY, title='Version Info',),
        )
...
class ViewWithDateTimeAndNumRunsAndDagRunsFormTester:
    ...
    def assert_run_is_in_dropdown_not_selected(self, run, data):
        self.test.assertIn(
            '<option value="{}">{}</option>'.format(run.execution_date.isoformat(), run.run_id), data
        )

    def assert_run_is_selected(self, run, data):
        self.test.assertIn(
            '<option selected value="{}">{}</option>'.format(run.execution_date.isoformat(), run.run_id), data
        )
...
class TestDagACLView(TestBase):
    ...
    def login(self, username=None, password=None):
        ...
        return self.client.post('/login/', data=dict(username=user, password=passwd))
    ...
    def add_permission_for_role(self):
        ...
        perm_on_dag = self.appbuilder.sm.find_permission_view_menu('can_dag_edit', 'example_bash_operator')
        ...
        perm_on_all_dag = self.appbuilder.sm.find_permission_view_menu('can_dag_edit', 'all_dags')
        ...
        read_only_perm_on_dag = self.appbuilder.sm.find_permission_view_menu(
            'can_dag_read', 'example_bash_operator'
        )
    ...
    def test_success_fail_for_read_only_role(self):
        ...
        self.logout()
        self.login(username='dag_read_only', password='dag_read_only')
...
class TestExtraLinks(TestBase):
    def setUp(self):
        ...
        class FooBarLink(BaseOperatorLink):
            ...
            def get_link(self, operator, dttm):
                return 'http://www.example.com/{0}/{1}/{2}'.format(operator.task_id, 'foo-bar', dttm)
    ...
    def test_extra_links_works(self):
        response = self.client.get(
            "{0}?dag_id={1}&task_id={2}&execution_date={3}&link_name=foo-bar".format(
    ...
    def test_global_extra_links_works(self):
        response = self.client.get(
            "{0}?dag_id={1}&task_id={2}&execution_date={3}&link_name=github".format(
                self.endpoint, self.dag.dag_id, self.task.task_id, self.default_date
            ),
            follow_redirects=True,
        )
    ...
    def test_extra_links_error_raised(self):
        response = self.client.get(
            "{0}?dag_id={1}&task_id={2}&execution_date={3}&link_name=raise_error".format(
                self.endpoint, self.dag.dag_id, self.task.task_id, self.default_date
            ),
            follow_redirects=True,
        )
    ...
    def test_extra_links_no_response(self):
        response = self.client.get(
            "{0}?dag_id={1}&task_id={2}&execution_date={3}&link_name=no_response".format(
                self.endpoint, self.dag.dag_id, self.task.task_id, self.default_date
            ),
            follow_redirects=True,
        )
