...
class TestImpersonation(unittest.TestCase):
    def setUp(self):
        ...
        self.dagbag = models.DagBag(dag_folder=TEST_DAG_FOLDER, include_examples=False,)
    ...
    def test_no_impersonation(self):
        ...
        """
        self.run_backfill(
            'test_no_impersonation', 'test_superuser',
        )
...
class TestImpersonationWithCustomPythonPath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        grant_permissions()
        add_default_pool_if_not_exists()
        self.dagbag = models.DagBag(dag_folder=TEST_DAG_CORRUPTED_FOLDER, include_examples=False,)
        logger.info('Loaded DAGS:')
        logger.info(self.dagbag.dagbag_report())
