...
class TestGoogleDisplayVideo360GetReportOperator(TestCase):
    ...
    def test_execute(
        ...
        op.execute(context=None)
        mock_hook.assert_called_once_with(gcp_conn_id=GCP_CONN_ID, delegate_to=None, api_version=API_VERSION)
        mock_hook.return_value.get_query.assert_called_once_with(query_id=report_id)

        mock_gcs_hook.assert_called_once_with(google_cloud_storage_conn_id=GCP_CONN_ID, delegate_to=None)
...
class TestGoogleDisplayVideo360SDFtoGCSOperator(TestCase):
    ...
    def test_execute(self, mock_temp, gcs_mock_hook, mock_hook):
        ...
        gcs_mock_hook.assert_called_once_with(gcp_conn_id=GCP_CONN_ID, delegate_to=DELEGATE_TO)
